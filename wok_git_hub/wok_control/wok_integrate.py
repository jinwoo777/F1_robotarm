"""wok_integrate — probe_flip(부침개) + wok_final(볶음밥 웍질 디벨롭) + wok_test4(예외처리) 통합.

ROS 파라미터 `dish`로 조리를 선택한다 (기본값 fried_rice):
    ros2 run rokey wok_integrate --ros-args -p dish:=fried_rice
    ros2 run rokey wok_integrate --ros-args -p dish:=jeon

┌─ fried_rice (wok_final 기준) ────────────────────────────────────────────┐
│ 재료 3종 투입 → 레버 열기 → 웍 파지 → 1차 웍질 → scoop(힘제어 국자삽입)   │
│ → 웍 재파지 → 본 웍질(WOK_OUTER×WOK_INNER) → 붓기 → 웍 원위치 → 레버 닫기 │
└───────────────────────────────────────────────────────────────────────────┘
┌─ jeon (probe_flip 기준) ──────────────────────────────────────────────────┐
│ 재료투입/레버 없음 (이미 팬에 반죽이 있다고 가정) — 웍 파지 → flip 반복     │
│ (FLIP_COUNT) → 웍 원위치                                                  │
└───────────────────────────────────────────────────────────────────────────┘

두 조리 모두 wok_test4의 안전 인프라를 동일하게 사용한다:
  - check_estop_and_wait(): 모든 movel/movej 완료 직후 안전정지 감지 + 자동 해제/복구
  - RestartPreparation: 준비 단계 중 안전정지가 걸리면 그 단계만 재시작
  - 그리퍼 Modbus(RG2Client) grip_detected로 실제 파지 성공/실패 판정
  - /wok_stage, /tcp_pose, /gripper_width 퍼블리시 (backend 대시보드 연동, 토픽명 wok_test4와 동일)

probe_flip.py의 웍 파지 위치(704.84, -21.97, 181.70, 178.01, -109.75, 178.85)와 접근 관절
(-4.10, -9.12, 133.07, -1.39, -17.77, -0.13)은 wok_final의 P_WOK_GRIP / J_WOK_APPROACH와
정확히 동일해 그대로 재사용한다.

real_robot_execution 관련 주의: 원본 세 파일 중 어느 것도 자동 평가/시뮬레이션 검증 없이
바로 실기에 투입되도록 만들어지지 않았다. 처음 실행은 SPEED_RATIO/FLIP_SPEED_RATIO를
낮춰 경로부터 확인할 것.
"""
import time
...
# ▼▼▼ 볶음밥(fried_rice) 파라미터 — wok_final.py 기준 ▼▼▼
...
# ▼▼▼ 부침개(jeon) 파라미터 — probe_flip.py 기준 ▼▼▼
FLIP_SPEED_RATIO = 0.3          # 홈/파지 이동 속도 (원본 SPEED_RATIO)
FLIP_COUNT = 3
...
# ─────────────────────────────────────────────────────────────────────────
# 반복 좌표 상수 (wok_final.py 기준). main() 안에서 posj()/posx()로 래핑된다.
# ─────────────────────────────────────────────────────────────────────────
...
_P_SCOOP_TRANSIT_A  = [439.91, -356.18, 26.75, 136.39, -179.80, -130.78]
_P_SCOOP_TRANSIT_B  = [335.25, -356.98, 32.47, 168.59, 179.74, -98.19]

import DR_init
DR_init.__dsr__id = ROBOT_ID
DR_init.__dsr__model = ROBOT_MODEL
logger = get_logger("wok_integrate")
...
def main(args=None):
    ...
    node = rclpy.create_node("wok_integrate", namespace=ROBOT_ID)
    ...
    from DSR_ROBOT2 import (
        ...
        movej as _movej, movel as _movel, move_periodic, move_spiral,
    ...
    # 힘제어 상수는 설치 버전에 따라 없을 수 있어 방어적으로 처리 (wok_final과 동일 패턴).
    try:
    ...
    logger.info("wok_integrate 시작 — dish=%s, SPEED_RATIO=%.2f, FLIP_SPEED_RATIO=%.2f, SIMULATION=%s"
                % (dish, r, FLIP_SPEED_RATIO, SIMULATION))

    set_singular_handling(DR_AVOID)

    # ── 볶음밥(fried_rice) 서브루틴 (wok_final.py 기준) ────────────────────
    ...
    def scoop():
        """힘제어 기반 국자 삽입 (wok_final.py 원본 그대로).
          - move_spiral의 rad_dir/rot_dir/ra/radius 인자는 ROS2 API에 없어 제거된 상태 유지
          - check_force_condition 반환값: ROS2는 0=조건 충족, -1=미충족 (DRL과 반대이므로 == 0 비교)
          - SIMULATION=True면 힘제어 구간 전체 우회 (시뮬레이터엔 접촉 물리 없음)
        """
        movej(J_SCOOP_STANDBY, radius=0.00, ra=DR_MV_RA_DUPLICATE)
        movej(posj(-39.69, 32.44, 78.09, -0.24, 69.01, 53.35),
              radius=0.00, ra=DR_MV_RA_DUPLICATE)
        grip_close()
        wait(2.00)
        ...
        if SIMULATION:
            ...
            set_desired_force([0.00, 0.00, -5.00, 0.00, 0.00, 0.00],
                               [0, 0, 1, 0, 0, 0], time=0.0, mod=_DR_FC_MOD_REL)
            wait(10.00)
            fc = check_force_condition(axis=_DR_AXIS_Z, min=2, max=8, ref=_DR_BASE)
            logger.info("scoop 힘 조건 검사 결과 ret=%s (0=충족)" % fc)
            if fc == 0:
                move_spiral(rev=3.00, rmax=40.00, lmax=20.00, time=8.00,
                            axis=_DR_AXIS_Z, ref=0)
    ...
    def wok_pick():
        """웍 파지 — 반드시 P_WOK_GRIP에서. 그리퍼 Modbus 파지검증(wok_test4 이식) 포함."""
    ...
    def wok_place():
        """웍 내려놓기 — 파지 좌표와 동일한 P_WOK_GRIP으로 통일."""
        movel(P_WOK_GRIP, radius=0.00, ref=0, mod=DR_MV_MOD_ABS, ra=DR_MV_RA_DUPLICATE)
        set_digital_output(1, OFF)
        set_digital_output(2, ON)
        wait(GRIP_WAIT)
    ...
    def wokking():
        ...
        movej(posj(11.32, -44.41, 118.37, 3.22, 36.75, -7.76),
              radius=0.00, ra=DR_MV_RA_DUPLICATE)
        movel(posx(413.20, 176.55, 392.31, 20.32, 115.00, -115.32),
              radius=0.00, ref=0, mod=DR_MV_MOD_ABS, ra=DR_MV_RA_DUPLICATE)
    ...
    # ── 부침개(jeon) 서브루틴 (probe_flip.py 기준) ─────────────────────────

    def jeon_flip():
        """뒤집기 반복. 원본과 달리 모든 movel/movej가 estop 체크포인트를 포함한 공통
        래퍼를 지나간다 — 실제 하드웨어에서 도는 동작이라 안전정지 감지가 특히 중요하다는
        판단(요청 반영)."""
        for i in range(FLIP_COUNT):
            publish_stage("뒤집기 %d/%d" % (i + 1, FLIP_COUNT))
            movej(READY_J, vel=VELOCITY_J, acc=ACC_J, radius=0, ra=DR_MV_RA_DUPLICATE)
            wait(0.3)
            movej(FLAP_J, vel=WOK_VEL_J, acc=WOK_ACC_J, radius=0, ra=DR_MV_RA_DUPLICATE)
            wait(0.05)
            movej(FLAT_J, vel=WOK_VEL_J, acc=WOK_ACC_J, radius=0, ra=DR_MV_RA_DUPLICATE)
            wait(0.1)
            movel(P_WOK_GRIP, radius=0.00, ref=0, mod=DR_MV_MOD_ABS, ra=DR_MV_RA_DUPLICATE)
            if i < FLIP_COUNT - 1:

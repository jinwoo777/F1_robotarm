...
# 저장소 안에서의 상대 위치로 ROS 워크스페이스를 찾는다 (backend/ 의 형제 폴더 ws_dsr).
# 환경변수로 덮어쓸 수 있게 해둬서, 워크스페이스를 저장소 밖에 두고 쓰는 경우도 지원한다.
WS_DSR_DIR = os.environ.get(
    "WOK_WS_DSR_DIR", os.path.join(os.path.dirname(BASE_DIR), "ws_dsr"))
WS_DSR_SETUP = os.path.join(WS_DSR_DIR, "install", "setup.bash")
# 로봇/그리퍼와 같은 도메인이어야 통신이 된다. 기본 0 이 아니라 50 을 쓰고 있음에 유의.
ROS_DOMAIN_ID = os.environ.get("ROS_DOMAIN_ID", "50")
...
def is_wok_integrate_running():
    ...
    검색어 "rokey/lib/rokey/wok_integrate"는 접두사라 wok_integrate / 2 / 3 을 모두 잡는다 —
    의도한 것이다. 버전이 달라도 제어하는 로봇은 하나뿐이라, 예전 버전이 떠 있는 상태에서
    새 버전이 또 실행되면 안 된다.
...
def admin_orders():
    # 관리자 화면 "주문 관리" 탭이 몇 초 간격으로 폴링해서, 손님이 폰으로 주문을 넣거나
    # 로봇이 자동으로 조리 완료 처리를 해도 관리자가 새로고침하지 않아도 반영되게 한다.
...
def update_status():
    ...
    if status == "COOKING":
        if is_wok_integrate_running():
            ...
            return jsonify({"status": "error", "message": "이미 조리 중인 로봇 작업이 있습니다. 완료된 뒤에 다시 시작해주세요."})
        ...
        # 워크스페이스 경로는 이 파일 위치에서 계산한다 — 절대경로를 박아두면 저장소를
        # 다른 곳에 clone 했을 때 source 가 실패해 조리가 아예 안 뜬다.
        # DSR_ROBOT2(dsr_common2/imp)는 ament 로 설치되지 않아 PYTHONPATH 로 직접 잡아줘야 하는데,
        # 두산 패키지를 어디에 빌드해뒀는지가 환경마다 달라서 후보 경로를 모두 붙인다(없는 건 무시됨).
        cmd = f"""
        source /opt/ros/humble/setup.bash
        source {WS_DSR_SETUP}
        export ROS_DOMAIN_ID={ROS_DOMAIN_ID}
        export PYTHONPATH=$PYTHONPATH:{WS_DSR_DIR}/install/dsr_common2/lib/dsr_common2/imp
        export PYTHONPATH=$PYTHONPATH:~/ros2_ws/install/dsr_common2/lib/dsr_common2/imp
        ros2 run rokey wok_integrate4 --ros-args -p dish:={dish}
        """
